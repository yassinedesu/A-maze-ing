...
class MazeGenerator:
    ...
    Example:
        ...
        >>> gen.generate()
        >>> print(gen.solution)   # list of 'N'/'E'/'S'/'W'
    ...
    def _embed_42_pattern(self, forbidden: set[tuple[int, int]]) -> None:
        ...
        Also closes the walls of all neighbouring traversable cells that face
        a pattern cell, to keep the grid coherent.
        ...
        # Set each pattern cell to ALL_WALLS
        for cx, cy in forbidden:
            self.grid[cy][cx] = ALL_WALLS

        # Close the wall on each neighbour that faces a pattern cell
        for cx, cy in forbidden:
    ...
    def _generate_dfs(self, forbidden: set[tuple[int, int]]) -> None:
        ...
        The algorithm visits every traversable cell exactly once, removing
        walls between cells to create a spanning tree (perfect maze).
        Cells in *forbidden* are treated as pre-visited obstacles.
    ...
    def _add_imperfections(self, forbidden: set[tuple[int, int]]) -> None:
        ...
        Args:
            forbidden: Cells to avoid (pattern cells, border openings).
        """
        ...
        while added < target and attempts < target * 20:
            ...
            if not self._in_bounds(nx, ny) or (nx, ny) in forbidden:
                continue
            # Never remove external border walls
            on_border = (
                (y == 0 and d == NORTH)
    ...
    def _is_3x3_open(self, ox: int, oy: int) -> bool:
        """Return True if the 3×3 block at (ox, oy) has no internal walls.

        A 3×3 block has 12 internal walls (6 East + 6 South).
    ...
    def _enforce_corridor_width(self, forbidden: set[tuple[int, int]]) -> None:
        """Add a wall in every fully open 3×3 block to limit corridor width.
        ...
        for y in range(self.height - 2):
            for x in range(self.width - 2):
                if not self._is_3x3_open(x, y):
                    continue
                # Candidate internal walls (avoid pattern cells)
                candidates = [
                    (x, y, EAST),
                    (x, y, SOUTH),
                    (x + 1, y, EAST),
    ...
    def generate(self) -> None:
        ...
        Steps:
            ...
            3. Run DFS (forbidden cells treated as visited obstacles).
            ...
            7. Eliminate fully open 3×3 areas.
        ...
        from mazegen.solver import bfs_solve  # noqa: PLC0415
    ...
    def _generate_prim(self, forbidden: set[tuple[int, int]]) -> None:
        ...
        Maintains a frontier of walls. At each step picks one at random,
        carves it if the neighbor is unvisited. Produces cave-like mazes
        with shorter corridors than DFS.
        ...
        # frontier = list of (from_x, from_y, direction)
